...
export default function SellerNotificationsMenu({ userId }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [list, unread] = await Promise.all([
        sellerNotificationService.listForSeller(userId),
        sellerNotificationService.unreadCount(userId),
      ]);
      setItems(list);
  ...
  useEffect(() => {
    ...
    refresh();
    const channel = sellerNotificationService.subscribeToSeller(userId, refresh);
    return () => {
  ...
  const routeFor = (item) => {
    if (item?.order_id) return `/seller/orders/${item.order_id}`;
    return '/seller';
  };
  ...
  const handleItemClick = async (item) => {
    ...
    try {
      if (!item.is_read) {
        await sellerNotificationService.markAsRead(item.id);
      }
      refresh();
      handleClose();
      navigate(routeFor(item));
    } catch (error) {
      console.error('Failed to open seller notification:', error);
    }
  };

  const handleMarkAllRead = async () => {
    try {
      await sellerNotificationService.markAllAsRead(userId);
      refresh();
    } catch (error) {
      console.error('Failed to mark seller notifications as read:', error);
    }
  };

  const emptyText = useMemo(() => (loading ? 'Loading notifications...' : 'No seller notifications yet'), [loading]);
...
SellerNotificationsMenu.propTypes = {
  userId: PropTypes.string,
};

SellerNotificationsMenu.defaultProps = {
  userId: '',
};
