...
export default function AccountSidebar() {
  ...
  return (
    <aside className="space-y-4 lg:sticky lg:top-36 lg:self-start">
      ...
      {profile && (
        <div className="flex items-center gap-3 p-3 bg-white rounded-xl border border-outline-variant/15">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-on-surface-variant/50 font-body truncate">{profile.email}</p>
          </div>
        </div>
      )}
            ...
            <Link key={link.label} to={link.href}
              className={`group flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all font-body text-sm ${
                active 
                  ...
                  : `hover:bg-surface-container-low ${link.special ? 'text-secondary font-semibold' : 'text-on-surface-variant hover:text-on-surface'}`
              }`}>
              <span className="material-symbols-outlined text-[18px]" style={active ? { fontVariationSettings: "'FILL' 1" } : {}}>{link.icon}</span>
      ...
      {/* Help card — hidden on mobile, very subtle */}
      <div className="hidden lg:block bg-surface-container-low rounded-lg p-4 border border-outline-variant/10">
        ...
        <p className="text-xs text-on-surface-variant/60 font-body mb-3 leading-relaxed">Our team is available to assist with your orders.</p>
        <Link to="/contact">
