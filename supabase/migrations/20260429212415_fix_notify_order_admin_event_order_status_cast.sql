-- Fix enum cast failure in order notification trigger.
-- NEW.order_status / OLD.order_status are enums, so COALESCE(..., '') can try
-- to cast '' to enum and fail. Cast enum columns to text before COALESCE.
