-- admin_finalize_order must set hatvoni.workflow_actor so trg_guard_order_status_change
-- allows pending → processing | rejected | partially_approved (guard checks this before hard-coded rules).
...
COMMENT ON FUNCTION public.admin_finalize_order(uuid, text, text) IS
  'Advances a pending order after all item decisions; sets hatvoni.workflow_actor for guard_order_status_change.';
